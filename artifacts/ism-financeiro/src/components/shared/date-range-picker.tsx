    ...
    return (
        <input
            ...
            onBlur={() => {
                ...
                setText(parsed ? formatDateBr(parsed) : formatDateBr(value));
            }}
            className="bg-transparent border-none outline-none text-white w-full pl-6 cursor-text placeholder:text-muted-foreground/50"
            lang="pt-BR"
        />
    );
}
             ...
             {format(date, "MMMM yyyy", {locale: ptBR})}
                ...
                <div className="grid grid-cols-7 gap-0.5">
                    {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sab"].map(d => (
                        <div key={d}
                             className="text-[9px] font-bold text-muted-foreground/50 text-center py-1 uppercase">{d}</div>
                    ))}
                    {padding}
                    {days.map(day => {
        ...
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            <div className="flex items-center gap-2">
                <PopoverTrigger asChild>
                    ...
                    >
                        ...
                        {startDate ? format(new Date(startDate + "T00:00:00"), "dd/MM/yyyy", {locale: ptBR}) : "Início"}
                        <span className="text-muted-foreground/40">até</span>
                        {endDate ? format(new Date(endDate + "T00:00:00"), "dd/MM/yyyy", {locale: ptBR}) : "Fim"}
                    </button>
                </PopoverTrigger>
                {(startDate || endDate) && (
                    <button
            ...
            >
                <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-6">
                    <div className="flex items-center gap-2">
                        <span
                            className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mr-2">Últimos</span>
                        {[7, 15, 30].map(d => (
                            <button key={d} onClick={() => handlePreset(d)}
                                    className="px-3 py-1 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-bold text-white transition-all">
                                {d} Dias
                            </button>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-12 gap-8">
                    <div className="col-span-8 flex flex-col gap-6">
                        <div className="flex items-center gap-4">
                            <div className="flex-1 space-y-1.5">
                                <label
                                    className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">De</label>
                                <div
                                    className="flex items-center gap-2 p-2 bg-black/40 border border-white/5 rounded-xl text-xs font-bold text-white relative">
                            ...
                            <div className="flex-1 space-y-1.5">
                                <label
                                    className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Até</label>
                                <div
                                    className="flex items-center gap-2 p-2 bg-black/40 border border-white/5 rounded-xl text-xs font-bold text-white relative">
                                    <CalendarIcon className="w-3.5 h-3.5 text-primary absolute left-3"/>
                                    <BrDateInput
                    ...
                    <div className="col-span-4 border-l border-white/5 pl-8 space-y-6">
                        <div className="flex items-center justify-between">
                            <button onClick={() => setViewDate(setYear(viewDate, viewDate.getFullYear() - 1))}
                                    className="p-1 hover:bg-white/10 rounded-lg text-white">
                                <ChevronLeft className="w-4 h-4"/>
                            </button>
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/5">
                    <button onClick={() => setIsOpen(false)}
                            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-white/5 hover:bg-destructive/10 text-white hover:text-destructive text-xs font-black transition-all">
                        <X className="w-3.5 h-3.5"/> CANCELAR
                    </button>
                    <button onClick={handleApply}
                            className="flex items-center gap-2 px-8 py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-white text-xs font-black shadow-lg shadow-primary/20 transition-all">
