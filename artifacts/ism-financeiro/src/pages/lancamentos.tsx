...
const RISCO_STYLE: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  "Restrição de Crédito":        { label: "Restrição de Crédito",        color: "#FBBF24", bg: "rgba(251,191,36,0.15)",  border: "rgba(251,191,36,0.35)"  },
  "Corte de Serviço":            { label: "Corte de Serviço",            color: "#FB923C", bg: "rgba(251,146,60,0.15)",  border: "rgba(251,146,60,0.35)"  },
  "Suspensão de Fornecimento":   { label: "Suspensão de Fornecimento",   color: "#FB923C", bg: "rgba(251,146,60,0.15)",  border: "rgba(251,146,60,0.35)"  },
  "Negativação":                 { label: "Negativação",                 color: "#FB923C", bg: "rgba(251,146,60,0.15)",  border: "rgba(251,146,60,0.35)"  },
  "Perda de Benefício Fiscal":   { label: "Perda de Benefício Fiscal",   color: "#FB923C", bg: "rgba(251,146,60,0.15)",  border: "rgba(251,146,60,0.35)"  },
  "Protesto":                    { label: "Protesto",                    color: "#F87171", bg: "rgba(248,113,113,0.15)", border: "rgba(248,113,113,0.35)" },
  "Ação Judicial":               { label: "Ação Judicial",               color: "#F87171", bg: "rgba(248,113,113,0.15)", border: "rgba(248,113,113,0.35)" },
  "Dívida Ativa":                { label: "Dívida Ativa",                color: "#F87171", bg: "rgba(248,113,113,0.15)", border: "rgba(248,113,113,0.35)" },
  "Quebra de Contrato":          { label: "Quebra de Contrato",          color: "#F87171", bg: "rgba(248,113,113,0.15)", border: "rgba(248,113,113,0.35)" },
  "Bloqueio de Contas (Sisbajud)":{ label: "Bloqueio (Sisbajud)",        color: "#C084FC", bg: "rgba(192,132,252,0.15)", border: "rgba(192,132,252,0.35)" },
  "Penhora de Bens":             { label: "Penhora de Bens",             color: "#C084FC", bg: "rgba(192,132,252,0.15)", border: "rgba(192,132,252,0.35)" },
  "Pedido de Falência":          { label: "Pedido de Falência",          color: "#C084FC", bg: "rgba(192,132,252,0.15)", border: "rgba(192,132,252,0.35)" },
  "Impedimento de Certidão":     { label: "Impedimento de Certidão",     color: "#C084FC", bg: "rgba(192,132,252,0.15)", border: "rgba(192,132,252,0.35)" },
};
              ...
              <div className="grid grid-cols-2 gap-4">
                    ...
                    <Popover open={vctoOpen} onOpenChange={setVctoOpen}>
                      ...
                      <PopoverContent className="w-auto p-0 border border-white/10" align="start">
                        <CalendarPicker mode="single"
                          ...
                          onSelect={(date) => { field.onChange(date ? formatBtn(date, "yyyy-MM-dd") : ""); setVctoOpen(false); }}
                ...
                <div>
                  ...
                  <Controller name="competencia" control={control} render={({ field }) => (
                    <CompetenciaPicker value={field.value ?? ""} onChange={field.onChange} error={errors.competencia?.message} />
                  )} />
                </div>
              </div>
                    ...
                    <div className="space-y-4 animate-in pt-2">
                      <div className="flex items-center justify-between border-b border-white/5 pb-2">
                        ...
                        <button type="button" onClick={() => setShowAddTag(!showAddTag)}
                          ...
                          <Plus className={cn("w-2.5 h-2.5 transition-transform", showAddTag && "rotate-45")} />
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <DateRangePicker startDate={dateStart} endDate={dateEnd}
                ...
                onChange={(start: string, end: string) => { setDateStart(start); setDateEnd(end); setPage(1); }} />
            </div>
          </div>
