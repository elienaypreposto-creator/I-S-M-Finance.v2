...
function LancamentoModal({
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      const url = editItem
        ? `${API_URL}/lancamentos/${editItem.id}`
        : `${API_URL}/lancamentos`;
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  className={selectCls(!!errors.plano_conta_id)}
                >
                  <option value="">Indique a categoria contábil...</option>
                  {Array.isArray(planoContas) &&
                    ...
                    render={({ field }) => (
                      <div className="relative group">
                        <select
                          ...
                          className={cn(
                            selectCls(),
                            "border-white/5 bg-black/40 font-black tracking-tight hover:border-white/20",
      ...
      <div className="glass-panel rounded-2xl flex flex-col overflow-hidden flex-1 min-h-0">
        ...
        <div className="px-4 py-3 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-muted-foreground bg-black/10">
          <span className="order-2 sm:order-1">
            {isLoading
              ? "..."
              : `${(page - 1) * limit + 1}–${Math.min(page * limit, total)} de ${total.toLocaleString("pt-BR")} registros`}
          </span>
            ...
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-1 sm:flex-none px-3 py-2 rounded font-medium transition-colors",
                    pageNum === page ? "bg-primary text-white" : "hover:bg-white/5"
                  )}
                >
                  {pageNum}
                </button>
