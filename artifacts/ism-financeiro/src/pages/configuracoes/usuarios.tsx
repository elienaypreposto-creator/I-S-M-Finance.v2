                            ...
                            <div key={grupo.grupo} className="border border-white/5 rounded-xl overflow-hidden">
                                ...
                                >
                                    <div className="flex items-center gap-2 sm:gap-3 text-left">
                                        ...
                                        />
                                        <span
                                            className="font-semibold text-white text-xs sm:text-sm">{grupo.grupo}</span>
                                    ...
                                    <div className="divide-y divide-white/5">
                                        {grupo.itens.map((item) => (
                                            ...
                                            >
                                                ...
                                                />
                                                <span
                                                    className="text-xs sm:text-sm text-muted-foreground flex-1">{item.nome}</span>
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            {isOpen && search.length >= 3 && !disabled && (
                <div
                    className="absolute top-full left-0 right-0 mt-2 bg-[#1A1A24] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden max-h-60 overflow-y-auto">
                    {isLoading ? (
                        <div
                            className="p-3 text-xs text-muted-foreground text-center flex items-center justify-center gap-2">
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div
                ...
                <div
                    className="flex items-center justify-between p-4 sm:p-6 border-b border-white/5 sticky top-0 bg-card z-10">
                ...
                <form onSubmit={handleSubmit(onSave)} noValidate>
                    <div className="p-4 sm:p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input {...register("email")} type="email" disabled={isEdit}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label
                                    className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1.5 block">Cargo</label>
                                <input {...register("cargo")} className={inputCls(!!e.cargo)}
                                       placeholder="Ex: Analista"/>
                                <FieldError message={e.cargo?.message}/>
                            </div>
                            <div>
                                <label
                                    className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1.5 block">Perfil
                                    Base <span className="text-destructive">*</span></label>
                                <select
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <Controller name="telefone" control={control} render={({field}) => (
                                    <input value={field.value ?? ""}
                                           ...
                                           className={inputCls(!!e.telefone)} placeholder="(11) 3456-7890"
                                           inputMode="tel"/>
                                )}/>
                            ...
                            <div>
                                <label
                                    className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1.5 block">Celular</label>
                                <Controller name="celular" control={control} render={({field}) => (
                                    <input value={field.value ?? ""}
                                           onChange={(ev) => field.onChange(mascararTelefone(ev.target.value))}
                                           className={inputCls(!!e.celular)} placeholder="(11) 9 9999-9999"
                                           inputMode="tel"/>
                                )}/>
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            {tab === "usuarios" && (
                <>
                    ...
                    {isError && !isLoading && (
                        <div
                            className="glass-panel rounded-2xl p-6 border border-destructive/20 flex flex-col items-center justify-center text-center gap-3 bg-destructive/5">
                            <div
                                className="w-12 h-12 rounded-full bg-destructive/10 flex items-center justify-center text-destructive mb-2">
                    ...
                    {!isLoading && !isError && (
                        <>
                            <div className="flex gap-4 text-xs text-muted-foreground">
                                <span><span
                                    className="font-semibold text-white">{usuarios.length}</span> usuário{usuarios.length !== 1 ? "s" : ""}</span>
                                <span className="text-success"><span
                                    className="font-semibold">{ativos}</span> ativo{ativos !== 1 ? "s" : ""}</span>
                                <span className="text-muted-foreground/60"><span
                                    className="font-semibold">{usuarios.length - ativos}</span> bloqueado{usuarios.length - ativos !== 1 ? "s" : ""}</span>
                            </div>
                                    ...
                                    <div className="glass-panel rounded-2xl overflow-hidden hidden md:block">
                                        <table className="w-full text-sm">
                                            ...
                                            <tr>
                                                ...
                                                <th className="px-5 py-3 text-center font-medium text-muted-foreground">Último
                                                    Acesso
                                                </th>
                                                ...
                                                <tr key={u.id} className="hover:bg-white/5 transition-colors group">
                                                    <td className="px-5 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <div
                                                                className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary/40 to-primary/20 flex items-center justify-center font-bold text-xs text-primary shrink-0">
                                                                {getInitials(u.nome)}
                                                            </div>
                                                            <div>
                                        ...
                                        {usuarios.map((u) => (
                                            <div key={u.id} className="glass-panel rounded-2xl p-4">
                                                <div className="flex items-start gap-3">
                                                    <div
                                                        className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/40 to-primary/20 flex items-center justify-center font-bold text-xs text-primary shrink-0">
                                                        {getInitials(u.nome)}
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between gap-2">
                                                            ...
                                                            {!u.bloqueado ? (
                                                                <span
                                                                    className="inline-flex items-center gap-1 text-xs text-success shrink-0">
                                  <CheckCircle className="w-3 h-3"/> Ativo
                                </span>
                                                            ) : (
                                                                <span
                                                                    className="inline-flex items-center gap-1 text-xs text-destructive shrink-0">
