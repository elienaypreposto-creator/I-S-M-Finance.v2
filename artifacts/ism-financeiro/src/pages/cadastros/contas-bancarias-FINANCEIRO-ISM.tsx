...
const ACCOUNT_TYPES = [
  { value: "Conta Corrente", label: "Conta Corrente", Icon: Landmark },
  { value: "Conta Caixinha", label: "Conta Caixinha", Icon: PackageOpen },
  { value: "Conta PJ", label: "Conta PJ", Icon: Building2 },
  { value: "Conta Poupança", label: "Conta Poupança", Icon: PiggyBank },
  { value: "Investimento", label: "Investimento", Icon: TrendingUp },
] as const;

// Tipos que não exigem agência/conta bancária
const TIPOS_SEM_AGENCIA_CONTA = ["Investimento", "Conta Caixinha"];
...
const BANCOS_BR = [
  { codigo: "001", nome: "Banco do Brasil", sigla: "BB", cor: "#0033A0", texto: "#FFEF00", logo: "/logos-bancos/001.svg" },
  { codigo: "033", nome: "Santander", sigla: "SA", cor: "#EC0000", texto: "#FFFFFF", logo: "/logos-bancos/033.svg" },
  { codigo: "104", nome: "Caixa Econômica Federal", sigla: "CX", cor: "#0070AD", texto: "#FF6600", logo: "/logos-bancos/104.svg" },
  { codigo: "237", nome: "Bradesco", sigla: "BR", cor: "#CC092F", texto: "#FFFFFF", logo: "/logos-bancos/237.svg" },
  { codigo: "341", nome: "Itaú Unibanco", sigla: "IT", cor: "#EC7000", texto: "#003087", logo: "/logos-bancos/341.svg" },
  ...
  { codigo: "290", nome: "PagBank (PagSeguro)", sigla: "PB", cor: "#00C650", texto: "#FFFFFF", logo: "/logos-bancos/290.svg" },
  ...
  { codigo: "070", nome: "BRB - Banco de Brasília", sigla: "BRB", cor: "#00833E", texto: "#FFC629", logo: "/logos-bancos/070.svg" },
  { codigo: "212", nome: "Banco Original", sigla: "OR", cor: "#00D084", texto: "#0B3D2E", logo: "/logos-bancos/212.svg" },
  { codigo: "655", nome: "Banco Votorantim (BV)", sigla: "BV", cor: "#FF5F00", texto: "#FFFFFF", logo: "/logos-bancos/655.svg" },
  { codigo: "208", nome: "Banco BTG Pactual", sigla: "BTG", cor: "#0A0A0A", texto: "#FFFFFF", logo: "/logos-bancos/208.svg" },
  { codigo: "197", nome: "Stone Pagamentos", sigla: "ST", cor: "#00A868", texto: "#FFFFFF", logo: "/logos-bancos/197.svg" },
  { codigo: "380", nome: "PicPay", sigla: "PP", cor: "#21C25E", texto: "#FFFFFF", logo: "/logos-bancos/380.svg" },
  { codigo: "102", nome: "XP Investimentos", sigla: "XP", cor: "#000000", texto: "#FFCE00", logo: "/logos-bancos/102.svg" },
] as const;
...
function BancoSelect({
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute z-50 mt-1.5 w-full max-h-64 overflow-y-auto bg-[#1a1b23] border border-white/10 rounded-xl shadow-2xl py-1.5">
          {BANCOS_BR.map((b) => (
            <button
              key={b.codigo}
              type="button"
...
function NovaContaModal({ onClose, initialData }: ModalProps) {
  ...
  // data_inicio é gerenciado fora do schema do form
  ...
  const { tipo, cor, nome, banco, agencia, conta, saldo_inicial_br } =
    formValues;
  ...
  function getStep2RequiredFields(): (keyof ContaBancariaFormValues)[] {
    const base: (keyof ContaBancariaFormValues)[] = ["nome", "banco"];
    if (TIPOS_SEM_AGENCIA_CONTA.includes(tipo)) {
      return base; // Poupança / Investimento / Caixinha: sem agência e conta
    }
    return [...base, "agencia", "conta"];
  }
  ...
  const step3Valid = !!dataInicio && !!saldo_inicial_br?.trim();
  ...
  const handleSave = async () => {
    // Validar data_inicio
    if (!dataInicio) {
      setDataInicioError("Campo obrigatório.");
      return;
    }
    setDataInicioError("");

    // Re-validar todos os campos obrigatórios como segurança
    const requiredFields: (keyof ContaBancariaFormValues)[] = [
      "nome",
      "banco",
      "saldo_inicial_br",
      ...(TIPOS_SEM_AGENCIA_CONTA.includes(tipo)
        ? []
        : (["agencia", "conta"] as const)),
    ];
    ...
    if (!tipo || !isValid || !step3Valid) {
      toast({
        ...
        description:
          "Há campos obrigatórios não preenchidos, volte e conclua!",
        variant: "destructive",
      });
      return;
    }
  ...
  const step1Summary =
    ACCOUNT_TYPES.find((t) => t.value === tipo)?.label ?? tipo;
  ...
  function getNomePlaceholder() {
    switch (tipo) {
      case "Poupança":
        return "Nome Conta Poupança";
      case "Investimento":
        return "Ex: Fundo Bradesco";
      case "Conta Caixinha":
        return "Nome conta Caixinha";
      default:
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
        {/* Header fixo */}
        ...
        <div className="overflow-y-auto flex-1">
          ...
          >
                ...
                {ACCOUNT_TYPES.map(({ value, label, Icon }) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex-1 min-w-[150px] flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl border-2 text-left transition-all",
                        selected
            ...
            >
              {!TIPOS_SEM_AGENCIA_CONTA.includes(tipo) ? (
                <>
                  {/* Banco, Agência, Conta e Nome — um abaixo do outro */}
                ...
                <>
                  {/* Poupança / Investimento / Caixinha: só Banco */}
                  <Field label="Banco" required error={errors.banco?.message}>
                    <Controller
                      name="banco"
                      control={control}
            ...
            >
              ...
              <div className="border border-primary/30 bg-primary/5 rounded-xl p-4 space-y-4">
                ...
                {/* Data de início */}
                <Field
                  label={`Início dos lançamentos${nome ? ` na Conta ${nome}` : ""}`}
                  required
                  error={dataInicioError}
                  hint={`Informe uma data até ${formatDateBr(maxDataInicio)}`}
                >
                  <input
                    type="date"
                    value={dataInicio}
                    ...
                    onChange={(e) => {
                      ...
                      if (novaData && novaData > maxDataInicio) {
                        // Bloqueia a seleção de datas futuras: não atualiza
                        // dataInicio, mantendo o último valor válido, e exibe
                        // a mensagem de validação.
                        setDataInicioError(
                          `Não é possível selecionar uma data futura. A data máxima permitida é ${formatDateBr(maxDataInicio)}.`,
                        );
                        return;
                      }
                      setDataInicio(novaData);
                      if (novaData) setDataInicioError("");
                    }}
                    className={inputClass}
                  />
                </Field>
