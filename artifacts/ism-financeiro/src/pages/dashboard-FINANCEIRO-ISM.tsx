...
// Ajustado para refletir o retorno real das rotas de inadimplência (agrupadas por parceiro)
type ParceiroInadimplente = { parceiro_id: number; nome: string; total: number; maior_dias_atraso: number; quantidade_titulos: number; };
// Ajustado a nomenclatura de dias_atraso para mapear com o backend
type AlertaRisco = { id: number; tipo: string; nome: string; dias_atraso: number; valor: number; riscos: string[]; };
...
function ContasPanel({ tipo, title, color, tab }: { tipo: "CP" | "CR"; title: string; color: "teal" | "orange"; tab: string }) {
  ...
  const { data: items = [], isLoading } = useQuery<ParceiroInadimplente[]>({
    queryKey: [`dashboard-${tipo}`, status, tab],
    // Extraindo o .data diretamente da resposta
    queryFn: () => fetchApi(endpoint).then((res: any) => res.data || []),
  });
                    ...
                    <div key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/5 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {c.maior_dias_atraso > 0 ? `${c.maior_dias_atraso}d de atraso máx · ` : ""}{c.quantidade_titulos} título(s)
                        </p>
                      </div>
                      <p className={`text-sm font-bold ${colorMap.val} shrink-0`}>
                        {tipo === "CP" ? "- " : ""}{formatCurrency(c.total)}
                      </p>
                    </div>
                );
              })
...
export default function Dashboard() {
  ...
  const { data: kpis, isLoading: kpisLoading } = useQuery<KPIs>({
    ...
    // Unwrap do payload
    queryFn: () => fetchApi("/dashboard/kpis").then((res: any) => res.data),
    refetchInterval: 60000,
  });

  const { data: projecao } = useQuery<ProjecaoMes>({
    queryKey: ["dashboard-projecao-mes"],
    queryFn: () => fetchApi("/dashboard/projecao-mes").then((res: any) => res.data),
  });

  const { data: fluxoCaixa = [], isLoading: fluxoLoading } = useQuery<FluxoMes[]>({
    queryKey: ["dashboard-fluxo", anoFluxo],
    queryFn: () => fetchApi(`/dashboard/fluxo-caixa-mensal?ano=${anoFluxo}`).then((res: any) => res.data || []),
  });

  const { data: alertasRisco = [] } = useQuery<AlertaRisco[]>({
    queryKey: ["dashboard-alertas-risco"],
    // Corrigido endpoint de /dashboard/nivel-risco para /dashboard/alertas-atraso
    queryFn: () => fetchApi("/dashboard/alertas-atraso?limit=100").then((res: any) => res.data || []),
  });

  const { data: saidasPlano = [] } = useQuery<PlanoItem[]>({
    queryKey: ["dashboard-saidas-plano"],
    queryFn: () => fetchApi("/dashboard/saidas-plano-contas").then((res: any) => res.data || []),
  });

  const { data: entradasPlano = [] } = useQuery<PlanoItem[]>({
    queryKey: ["dashboard-entradas-plano"],
    queryFn: () => fetchApi("/dashboard/entradas-plano-contas").then((res: any) => res.data || []),
  });

  const [filterLevel, setFilterLevel] = useState<number | "all">("all");

  const filteredAlertasRisco = alertasRisco.filter(a => {
    // Alertas de atraso já filtram CP no backend, mas mantemos por segurança
  ...
  return (
      <div className="space-y-5 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <ContasPanel tipo="CR" title="Contas a Receber (Por Parceiro)" color="teal" tab="vencidos" />
          <ContasPanel tipo="CP" title="Contas a Pagar (Por Fornecedor)" color="orange" tab="vencidos" />
        </div>

        {/* Alertas de Inadimplência e Risco */}
