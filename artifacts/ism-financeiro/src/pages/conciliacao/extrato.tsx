    ...
    <div className="flex flex-col gap-4 h-full max-w-6xl mx-auto py-2">
      ...
      {isLoading ? (
        ...
        <div className="glass-panel rounded-2xl p-10 border border-destructive/30 text-center">
          ...
          <button type="button" onClick={() => void refetch()} className="mt-4 text-xs text-primary underline">
            Tentar novamente
          </button>
        </div>
      ) : (
          ...
          <div className="glass-panel rounded-2xl p-6 border border-white/10 space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="space-y-2 min-w-0">
                ...
                <p className="text-sm text-muted-foreground">
                  Conta: <span className="text-white/90 font-medium">{extrato.conta_nome ?? "—"}</span>
                ...
                <p className="text-xs text-muted-foreground">Importado em {formatDate(extrato.created_at)}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2 border-t border-white/5">
              ...
              <button
                ...
                {finalizarMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                Finalizar conciliação
              </button>
            </div>
          </div>

          <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="px-4 py-3 border-b border-white/5 bg-black/20">
              <h2 className="text-sm font-bold text-white uppercase tracking-wide">Movimentações do extrato</h2>
              <p className="text-[11px] text-muted-foreground mt-0.5">Linhas importadas e vínculos com o financeiro</p>
            </div>
                    ...
                    <div className="flex flex-col xl:flex-row xl:items-stretch gap-4">
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded border", statusLinhaBadge(linha.status))}>
                            {linha.status}
                          </span>
                          {linha.data_movimento && (
                            <span className="text-[10px] text-muted-foreground">{formatDate(linha.data_movimento)}</span>
                          )}
                          {linha.documento && (
                            <span className="text-[10px] text-muted-foreground font-mono">Doc. {linha.documento}</span>
                          )}
                        </div>
                        ...
                        {!isPendente && (
                          <p className="text-[11px] text-muted-foreground">
                            Vinculado: {formatCurrency(Number(linha.valor_vinculado_total))} · Saldo linha:{" "}
                            {formatCurrency(Number(linha.valor_saldo))}
                          </p>
                        )}
                      </div>

                      <div className="flex flex-col sm:flex-row xl:flex-col gap-2 shrink-0 xl:w-56">
                        {isPendente && (
                          <>
                            <button
                              type="button"
                      ...
                      {isVinculado && linha.vinculacoes.length > 0 && (
                        <div className="xl:flex-1 xl:min-w-[280px] rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 space-y-2">
                            ...
                            {linha.vinculacoes.map((v) => (
                              <li
                                ...
                                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-muted-foreground">
                                  ...
                                  {(Number(v.desconto) > 0 || Number(v.acrescimo) > 0) && (
                                    <span>
                                      Desc./Acr.: {formatCurrency(Number(v.desconto))} / {formatCurrency(Number(v.acrescimo))}
                                    </span>
                                  )}
