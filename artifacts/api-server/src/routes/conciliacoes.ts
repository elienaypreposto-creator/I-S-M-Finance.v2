...
const buildDummyLinhas = () => {
  const baseDate = new Date();
  return [
    {
      identificador_externo: `dummy-${Date.now()}-1`,
      valor: "1850.00",
      tipo_movimento: "debito" as const,
      descricao: "PAGAMENTO FORNECEDOR A",
      data_movimento: baseDate.toISOString().split("T")[0],
      documento: "DOC-001",
    },
    {
      identificador_externo: `dummy-${Date.now()}-2`,
      valor: "4200.00",
      tipo_movimento: "credito" as const,
      descricao: "RECEBIMENTO CLIENTE B",
      data_movimento: new Date(baseDate.getTime() - 86400000).toISOString().split("T")[0],
      documento: "DOC-002",
    },
    {
      identificador_externo: `dummy-${Date.now()}-3`,
      valor: "950.00",
      tipo_movimento: "debito" as const,
      descricao: "TARIFA / SERVICOS BANCARIOS",
      data_movimento: new Date(baseDate.getTime() - 2 * 86400000).toISOString().split("T")[0],
      documento: "DOC-003",
    },
  ];
};
...
router.post("/conciliacoes/importar", upload.single("arquivo"), async (req, res) => {
  try {
    ...
    if (!req.file) {
      return errorResponse(res, 400, "VALIDATION_ERROR", "Campo obrigatório: arquivo.");
    }

    const linhasDummy = buildDummyLinhas();
    const periodoDatas = linhasDummy.map((l) => l.data_movimento).filter(Boolean) as string[];
    const periodoInicio = periodoDatas.slice().sort()[0] ?? null;
    const periodoFim = periodoDatas.slice().sort().reverse()[0] ?? null;
    const totalCreditos = linhasDummy
      .filter((l) => l.tipo_movimento === "credito")
      .reduce((acc, l) => acc + Number(l.valor), 0);
    const totalDebitos = linhasDummy
    ...
    const resultado = await db.transaction(async (tx) => {
      const [extrato] = await tx
        ...
        .values({
          ...
          total_linhas: linhasDummy.length,
          total_creditos: String(totalCreditos.toFixed(2)),
          total_debitos: String(totalDebitos.toFixed(2)),
        })
        .returning();
      ...
      const [conciliacao] = await tx
        ...
        .values({
          ...
          arquivo_nome: req.file?.originalname,
          status: "pendente",
          resumo_total: linhasDummy.length,
          resumo_pendentes: linhasDummy.length,
      ...
      await tx.insert(itensConciliacaoTable).values(
        linhas.map((linha) => ({
          ...
          valor_vinculado_total: "0",
          valor_saldo: linha.valor,
          status: "pendente",
          tipo_extrato: linha.tipo_movimento,
          descricao: linha.descricao,
          data: linha.data_movimento,
        })),
      );

      return { extrato, conciliacao, linhas };
    });

    return successResponse(res, resultado, null, 201);
  } catch (e) {
    return errorResponse(res, 500, "INTERNAL_ERROR", "Erro ao importar extrato.", String(e));
  }
});
...
router.post("/conciliacoes/linhas/:linha_id/vincular", async (req, res) => {
  try {
    ...
    const payloadMap = new Map(
      lancamentosPayload.map((l: { lancamento_id: number; desconto?: number; acrescimo?: number }) => [Number(l.lancamento_id), l]),
    );
