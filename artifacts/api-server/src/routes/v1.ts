...
router.get("/bancos", async (_req, res) => {
  try {
    ...
    return successResponse(res, items.map((i) => ({ ...i, saldo_inicial: Number(i.saldo_inicial ?? 0) })));
  } catch (e) {
...
router.get("/planoContas", async (_req, res) => {
  try {
    const items = await db.select().from(planoContasTable).orderBy(planoContasTable.categoria, planoContasTable.subcategoria);
    return successResponse(res, items);
  } catch (e) {
...
router.get("/tipoDocumentos", async (_req, res) => {
  try {
    const tipos = [
    ...
    return errorResponse(res, 500, "INTERNAL_ERROR", "Erro ao listar tipos de documento (v1).", String(e));
  }
});
