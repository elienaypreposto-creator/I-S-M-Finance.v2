...
const uniquePartners = [...new Set(data.map((r) => r.Parceiro).filter(Boolean))];
console.log('Total parceiros únicos:', uniquePartners.length);
console.log('Parceiros:', uniquePartners.join(', '));

const uniqueCategories = [...new Set(data.map((r) => r.Categoria).filter(Boolean))];
console.log('Categorias únicas:', uniqueCategories.join(', '));

const uniqueContas = [...new Set(data.map((r) => r.ContaBancaria).filter(Boolean))];
